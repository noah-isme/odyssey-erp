...
const elimGetRule = `-- name: ElimGetRule :one
...
func (q *Queries) ElimGetRule(ctx context.Context, id int64) (EliminationRule, error) {
	row := q.db.QueryRow(ctx, elimGetRule, id)
...
const elimInsertRule = `-- name: ElimInsertRule :one
...
type ElimInsertRuleParams struct {
...
func (q *Queries) ElimInsertRule(ctx context.Context, arg ElimInsertRuleParams) (EliminationRule, error) {
	row := q.db.QueryRow(ctx, elimInsertRule,
		arg.GroupID,
		arg.Name,
		arg.SourceCompanyID,
		arg.TargetCompanyID,
...
const elimListRecentPeriods = `-- name: ElimListRecentPeriods :many
...
type ElimListRecentPeriodsRow struct {
	ID        int64       `json:"id"`
...
func (q *Queries) ElimListRecentPeriods(ctx context.Context, limit int32) ([]ElimListRecentPeriodsRow, error) {
	rows, err := q.db.Query(ctx, elimListRecentPeriods, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ElimListRecentPeriodsRow
	for rows.Next() {
		var i ElimListRecentPeriodsRow
		if err := rows.Scan(
			&i.ID,
			&i.PeriodID,
			&i.Name,
...
const elimListRules = `-- name: ElimListRules :many
SELECT id, group_id, name, source_company_id, target_company_id, account_src, account_tgt, match_criteria,
       is_active, created_by, created_at, updated_at
...
func (q *Queries) ElimListRules(ctx context.Context, limit int32) ([]EliminationRule, error) {
	rows, err := q.db.Query(ctx, elimListRules, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const elimLoadAccountingPeriod = `-- name: ElimLoadAccountingPeriod :one
SELECT ap.id, ap.period_id, ap.name, ap.start_date, ap.end_date
FROM accounting_periods ap
WHERE ap.id = $1
`

type ElimLoadAccountingPeriodRow struct {
	ID        int64       `json:"id"`
	PeriodID  int64       `json:"period_id"`
	Name      string      `json:"name"`
	StartDate pgtype.Date `json:"start_date"`
...
func (q *Queries) ElimLoadAccountingPeriod(ctx context.Context, id int64) (ElimLoadAccountingPeriodRow, error) {
	row := q.db.QueryRow(ctx, elimLoadAccountingPeriod, id)
	var i ElimLoadAccountingPeriodRow
	err := row.Scan(
		&i.ID,
		&i.PeriodID,
		&i.Name,
