...
const rbacCreateRole = `-- name: RbacCreateRole :one
INSERT INTO roles (name, description)
VALUES ($1, $2)
RETURNING id, name, description, created_at, updated_at
`

type RbacCreateRoleParams struct {
	Name        string `json:"name"`
	Description string `json:"description"`
}

func (q *Queries) RbacCreateRole(ctx context.Context, arg RbacCreateRoleParams) (Role, error) {
	row := q.db.QueryRow(ctx, rbacCreateRole, arg.Name, arg.Description)
	var i Role
	err := row.Scan(
		&i.ID,
		&i.Name,
...
const rbacListRoles = `-- name: RbacListRoles :many
SELECT id, name, description, created_at, updated_at
FROM roles
ORDER BY name
`

func (q *Queries) RbacListRoles(ctx context.Context) ([]Role, error) {
	rows, err := q.db.Query(ctx, rbacListRoles)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
