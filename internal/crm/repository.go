...
func visibility(scope Scope, ownerColumn string) (string, []any) {
	if scope.ViewAll {
		return "company_id=$1", []any{scope.CompanyID}
	}
	return "company_id=$1 AND " + ownerColumn + "=$2", []any{scope.CompanyID, scope.UserID}
}
...
func (r *Repository) Pipeline(ctx context.Context, scope Scope) (Pipeline, error) {
	...
	where, args := visibility(scope, "o.owner_id")
	rows, err := r.pool.Query(ctx, `SELECT o.id,o.company_id,o.lead_id,o.contact_id,o.customer_id,o.quotation_id,o.owner_id,o.stage_id,o.name,o.source,ROUND(o.expected_value)::bigint,o.close_date,o.status,o.win_loss_reason,s.name,o.created_by,o.created_at,o.updated_at FROM crm_opportunities o JOIN crm_pipeline_stages s ON s.id=o.stage_id WHERE `+strings.ReplaceAll(where, "company_id", "o.company_id")+` ORDER BY s.position,o.close_date NULLS LAST,o.id`, args...)
	if err != nil {
		return out, err
	}
	defer rows.Close()
	...
	leadWhere, leadArgs := visibility(scope, "owner_id")
...
func (r *Repository) Lead(ctx context.Context, scope Scope, id int64) (Lead, error) {
	var x Lead
	where, args := visibility(scope, "owner_id")
	args = append(args, id)
...
func (r *Repository) Qualify(ctx context.Context, scope Scope, in QualifyInput) (out Opportunity, err error) {
	err = pgx.BeginTxFunc(ctx, r.pool, pgx.TxOptions{IsoLevel: pgx.Serializable}, func(tx pgx.Tx) error {
		...
		if lead.Status == "QUALIFIED" || lead.Status == "CONVERTED" {
			return scanOpportunity(tx.QueryRow(ctx, `SELECT o.id,o.company_id,o.lead_id,o.contact_id,o.customer_id,o.quotation_id,o.owner_id,o.stage_id,o.name,o.source,ROUND(o.expected_value)::bigint,o.close_date,o.status,o.win_loss_reason,s.name,o.created_by,o.created_at,o.updated_at FROM crm_opportunities o JOIN crm_pipeline_stages s ON s.id=o.stage_id WHERE o.lead_id=$1`, lead.ID), &out)
		}
		if lead.Status != "NEW" {
		...
		if e := tx.QueryRow(ctx, `INSERT INTO crm_contacts(company_id,lead_id,name,email,phone,created_by) VALUES($1,$2,$3,LOWER($4),$5,$6) RETURNING id`, lead.CompanyID, lead.ID, lead.Name, lead.Email, lead.Phone, in.ActorID).Scan(&contactID); e != nil {
		...
		return scanOpportunity(tx.QueryRow(ctx, `SELECT o.id,o.company_id,o.lead_id,o.contact_id,o.customer_id,o.quotation_id,o.owner_id,o.stage_id,o.name,o.source,ROUND(o.expected_value)::bigint,o.close_date,o.status,o.win_loss_reason,s.name,o.created_by,o.created_at,o.updated_at FROM crm_opportunities o JOIN crm_pipeline_stages s ON s.id=o.stage_id WHERE o.id=$1`, out.ID), &out)
	})
	if errors.Is(err, pgx.ErrNoRows) {
		err = ErrNotFound
	}
...
func (r *Repository) Opportunity(ctx context.Context, scope Scope, id int64) (Opportunity, error) {
	...
	where, args := visibility(scope, "o.owner_id")
	args = append(args, id)
	err := scanOpportunity(r.pool.QueryRow(ctx, `SELECT o.id,o.company_id,o.lead_id,o.contact_id,o.customer_id,o.quotation_id,o.owner_id,o.stage_id,o.name,o.source,ROUND(o.expected_value)::bigint,o.close_date,o.status,o.win_loss_reason,s.name,o.created_by,o.created_at,o.updated_at FROM crm_opportunities o JOIN crm_pipeline_stages s ON s.id=o.stage_id WHERE `+strings.ReplaceAll(where, "company_id", "o.company_id")+fmt.Sprintf(" AND o.id=$%d", len(args)), args...), &x)
	if errors.Is(err, pgx.ErrNoRows) {
		err = ErrNotFound
	}
	return x, err
...
func (r *Repository) Move(ctx context.Context, scope Scope, id int64, stage Stage, reason string, actor int64) (Opportunity, error) {
	...
	tag, err := r.pool.Exec(ctx, `UPDATE crm_opportunities SET stage_id=$`+fmt.Sprint(len(args)-3)+`,status=$`+fmt.Sprint(len(args)-2)+`,win_loss_reason=$`+fmt.Sprint(len(args)-1)+`,updated_at=NOW() WHERE `+where+fmt.Sprintf(" AND id=$%d AND status='OPEN'", len(args)), args...)
	if err != nil {
...
func (r *Repository) CompleteActivity(ctx context.Context, scope Scope, id, actor int64, at time.Time) error {
	...
	tag, err := r.pool.Exec(ctx, `UPDATE crm_activities SET completed_at=$`+fmt.Sprint(len(args)-1)+`,updated_at=$`+fmt.Sprint(len(args)-1)+` WHERE `+where+fmt.Sprintf(" AND id=$%d AND completed_at IS NULL", len(args)), args...)
	if err != nil {
		return err
	}
	if tag.RowsAffected() != 1 {
...
func (r *Repository) LinkCustomer(ctx context.Context, scope Scope, opportunityID, customerID, actor int64) error {
	return pgx.BeginTxFunc(ctx, r.pool, pgx.TxOptions{}, func(tx pgx.Tx) error {
		tag, err := tx.Exec(ctx, `UPDATE crm_opportunities o SET customer_id=$3,updated_at=NOW() WHERE o.id=$2 AND o.company_id=$1 AND ($4 OR o.owner_id=$5) AND (o.customer_id IS NULL OR o.customer_id=$3)`, scope.CompanyID, opportunityID, customerID, scope.ViewAll, scope.UserID)
		if err != nil {
			return err
		}
		if tag.RowsAffected() != 1 {
		...
		_, err = tx.Exec(ctx, `UPDATE crm_contacts c SET customer_id=$2,updated_at=NOW() FROM crm_opportunities o WHERE o.id=$1 AND c.id=o.contact_id`, opportunityID, customerID)
		return err
	})
}
func (r *Repository) LinkQuotation(ctx context.Context, scope Scope, opportunityID, quotationID, actor int64) error {
	tag, err := r.pool.Exec(ctx, `UPDATE crm_opportunities SET quotation_id=$3,updated_at=NOW() WHERE company_id=$1 AND id=$2 AND ($4 OR owner_id=$5) AND (quotation_id IS NULL OR quotation_id=$3)`, scope.CompanyID, opportunityID, quotationID, scope.ViewAll, scope.UserID)
	if err == nil && tag.RowsAffected() != 1 {
		return ErrNotFound
	}
	if err == nil {
		_, err = r.pool.Exec(ctx, `INSERT INTO crm_events(company_id,entity_type,entity_id,event_type,actor_id,details) VALUES($1,'OPPORTUNITY',$2,'CONVERTED',$3,jsonb_build_object('quotation_id',$4::bigint))`, scope.CompanyID, opportunityID, actor, quotationID)
	}
	return err
}
...
func (r *Repository) WinLoss(ctx context.Context, scope Scope) (WinLoss, error) {
	var out WinLoss
	where, args := visibility(scope, "owner_id")
	err := r.pool.QueryRow(ctx, `SELECT COUNT(*) FILTER(WHERE status='WON'),COALESCE(ROUND(SUM(expected_value) FILTER(WHERE status='WON')),0)::bigint,COUNT(*) FILTER(WHERE status='LOST'),COALESCE(ROUND(SUM(expected_value) FILTER(WHERE status='LOST')),0)::bigint FROM crm_opportunities WHERE `+where, args...).Scan(&out.WonCount, &out.WonValue, &out.LostCount, &out.LostValue)
	if err != nil {
		return out, err
	}
	rows, err := r.pool.Query(ctx, `SELECT COALESCE(NULLIF(win_loss_reason,''),'Unspecified'),COUNT(*),ROUND(SUM(expected_value))::bigint FROM crm_opportunities WHERE `+where+` AND status='LOST' GROUP BY 1 ORDER BY 2 DESC`, args...)
	if err != nil {
		return out, err
	}
	defer rows.Close()
