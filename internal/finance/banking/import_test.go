...
func TestParseCSVStatement(t *testing.T) {
	...
	if len(entries) != 2 || entries[0].Amount != 150000 || entries[1].Amount != -25000 || entries[0].Reference != "REF-1" {
...
func TestParseOFXStatement(t *testing.T) {
	...
	entries, err := parseStatement("statement.ofx", content)
	if err != nil {
		t.Fatalf("parseStatement() error = %v", err)
	}
	if len(entries) != 1 || entries[0].Amount != 12000.5 || entries[0].Reference != "OFX-1" {
		t.Fatalf("parseStatement() = %#v", entries)
	}
}
