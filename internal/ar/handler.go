...
func (h *Handler) showCustomerStatement(w http.ResponseWriter, r *http.Request) {
	// For now, list all invoices as a simple statement
	invoices, err := h.service.GetARInvoices(r.Context())
	if err != nil {
		h.render(w, r, "pages/ar/customer_statement.html", map[string]any{"Errors": formErrors{"general": err.Error()}}, http.StatusInternalServerError)
		return
	}
	h.render(w, r, "pages/ar/customer_statement.html", map[string]any{"Invoices": invoices}, http.StatusOK)
}
