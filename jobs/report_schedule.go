...
func HandleReportScheduleScanTask(logger *slog.Logger, db *pgxpool.Pool, client *asynq.Client) func(context.Context, *asynq.Task) error {
	return func(ctx context.Context, _ *asynq.Task) error {
		now := time.Now().UTC()
		rows, err := db.Query(ctx, `SELECT id, report_type, recipients, department_id, cost_center_id
		...
		for rows.Next() {
			...
			if err := rows.Scan(&id, &reportType, &recipients, &departmentID, &costCenterID); err != nil {
				return err
			}
			path := "/accounting/pnl"
