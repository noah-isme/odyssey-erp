...
| Distribution lifecycle | partial | partial | no | yes | `/distribution` -> `internal/distribution/handler.go` | Planning, loads, shipment linkage, dispatch/delivery, transfer foundations, and deterministic v1 route ordering/metrics are present; inventory/GL transfer posting, provider-backed routing, and operational workbenches remain. |
...
| RBAC and scoped access governance | yes | partial | no | yes | `/permissions` -> `internal/rbac/permissions_handler.go` | Permission middleware and module policies exist; effective-dated company/branch assignments, scoped middleware helpers, and company-scoped access-review persistence/service foundations now exist, while route adoption, migration/seed rollout, and the newer-module role matrix remain integration work. |
