...
func seedRBAC(ctx context.Context, pool *pgxpool.Pool) error {
	perms := []struct {
		...
		{"procurement.contract.terminate", "Terminate supplier contracts"},
		{"procurement.supplier_rating.view", "View supplier ratings"},
		{"procurement.supplier_rating.create", "Create supplier ratings"},
		{"procurement.supplier_rating.publish", "Publish supplier ratings"},
		{"procurement.price_history.view", "View supplier price history"},
		...
		{"finance.view_audit", "View Finance Audit Timeline"},
		...
		{"wms.view", "View warehouse execution"},
		{"wms.manage", "Manage bins and picking"},
		{"mrp.view", "View manufacturing planning"},
		{"mrp.manage", "Manage BOMs and work orders"},
		{"pos.view", "View point of sale"},
		{"pos.manage", "Manage terminals and POS sessions"},
		{"projects.view", "View projects and timesheets"},
		{"projects.manage", "Manage projects and approvals"},
	...
	roles := []struct {
		...
		{"admin", "Full access to all modules", []string{
			...
			"procurement.contract.view", "procurement.contract.manage", "procurement.contract.create", "procurement.contract.submit", "procurement.contract.approve", "procurement.contract.terminate",
			"procurement.supplier_rating.view", "procurement.supplier_rating.create", "procurement.supplier_rating.publish", "procurement.price_history.view", "procurement.variance.view", "procurement.variance.approve",
			...
			"wms.view", "wms.manage", "mrp.view", "mrp.manage", "pos.view", "pos.manage",
...
func seedListingFixtures(ctx context.Context, pool *pgxpool.Pool) error {
	...
	// Banking: give every seeded company an account and statement. The active
	// company can change between E2E sessions, so seeding only the first company
	// makes the statements route nondeterministic. bank_accounts carries no
	// unique constraint; the company/account-number predicate makes reruns safe.
	const seedBankAccountNumber = "1234567890"
	if glAccountID != 0 {
		rows, err := pool.Query(ctx, "SELECT id FROM companies ORDER BY id")
		if err != nil {
			return fmt.Errorf("list companies for bank fixtures: %w", err)
		}
		companyIDs := make([]int64, 0)
		for rows.Next() {
			var fixtureCompanyID int64
			if err := rows.Scan(&fixtureCompanyID); err != nil {
				rows.Close()
				return fmt.Errorf("scan company for bank fixtures: %w", err)
			}
			companyIDs = append(companyIDs, fixtureCompanyID)
		}
		...
		for _, fixtureCompanyID := range companyIDs {
			if _, err := pool.Exec(ctx, `
				...
				)`, fixtureCompanyID, glAccountID, seedBankAccountNumber); err != nil {
				return fmt.Errorf("bank account for company %d: %w", fixtureCompanyID, err)
			}
			var bankAccountID int64
			if err := pool.QueryRow(ctx, `
				SELECT id FROM bank_accounts
				WHERE company_id = $1 AND account_number = $2
				ORDER BY id LIMIT 1`, fixtureCompanyID, seedBankAccountNumber).Scan(&bankAccountID); err != nil {
				return fmt.Errorf("find bank account for company %d: %w", fixtureCompanyID, err)
			}
			if _, err := pool.Exec(ctx, `
				...
				return fmt.Errorf("bank statement for company %d: %w", fixtureCompanyID, err)
			}
		}
	}
