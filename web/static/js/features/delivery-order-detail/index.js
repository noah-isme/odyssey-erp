/**
 * Delivery Order Detail - State Driven Modals
 */

let state = {
    modals: {
        ship: { isOpen: false },
        deliver: { isOpen: false },
        cancel: { isOpen: false }
    }
};
...
function handleClick(e) {
    ...
    if (action === 'show-ship-modal') {
        dispatch({ type: 'OPEN_MODAL', payload: { name: 'ship' } });
    } else if (action === 'show-deliver-modal') {
        dispatch({ type: 'OPEN_MODAL', payload: { name: 'deliver' } });
    } else if (action === 'show-cancel-modal') {
